...
-- 1. Pobranie ID użytkownika i gry
SET @id_user = (SELECT id_uzytkownika FROM uzytkownik WHERE nazwa_uzytkownika = 'foo' LIMIT 1);
SET @id_game = (SELECT id_planszowki FROM planszowka WHERE tytul_planszowki = '7 Wonders' LIMIT 1);

-- 2. Dodanie definicji pluginu
-- POPRAWKA: Zmieniono 'nazwa' na 'nazwa_pluginu'. 
-- Jeśli nadal wystąpi błąd, sprawdź czy kolumna JSON nazywa się 'konfiguracja_json', 'json' czy 'tresc'.
...
-- Pobranie ID dodanego pluginu
SET @id_new_plugin = LAST_INSERT_ID();

-- 3. Powiązanie z grą (tu kolumny są poprawne wg pliku SQL)
